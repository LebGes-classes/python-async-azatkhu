...
class Menu:
    ...
    async def process_of_analysing(
        self, 
    ...
    ) -> AnalyzeMedicalDiagnosticDevices:
        ...
        await asyncio.to_thread(analyzer.dataframes_to_excel, [df1, df2, df3, df4], ['filtered_warranty', 'sorted_devices_issues', 'calibration_dates', 'table'], self.__count)
    ...
    async def user_choices(
        ...
        Запуск всех задач из списка analyzers одновременно и сбор всех результатов в список.
        ...
        Returns:
            __results: список результатов.
        """

        analyzers = [self.process_of_analysing(analyzer, choice_warranty_until_filter, choose_last_calibration_time, count_of_clinics) for analyzer in self.__analysing_devices]
        self.__results = await asyncio.gather(*analyzers)
