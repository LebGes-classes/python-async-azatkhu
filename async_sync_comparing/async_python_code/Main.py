...
if __name__ == '__main__':
    ...
    start = time.time()
    asyncio.run(menu.user_choices(choice_warranty_until_filter, choose_last_calibration_time, count_of_clinics))
    async_code_execution_time = time.time() - start
    with open('result_to_compare.txt', 'a', encoding = 'utf-8') as f:
        f.write('асинхронно_выполненный_код: ' + str(async_code_execution_time) + '\n') 

    print(f'Программа проработала {async_code_execution_time} секунд.')
